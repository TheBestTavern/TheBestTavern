...
public class MainTutorialController : BaseTutorialController
{
    ...
    private string[] mainScenetexts = { "안녕하세요 이 세계에 오신 용사... 아니 요리사님", "이곳에 갑자기 오시게 된 당신을 위해서", 
        ...
        "Tab을 눌러 현재 진행중인 의뢰를 포함한","여러가지 메뉴를 볼 수 있습니다." ,"의뢰에서 원하는 음식을 구체적으로 설명하지 않으니",
        "내용을 보고 잘 추측해 보시길 바랍니다.","이곳을 누르면 의뢰에 필요한 재료를 찾으러 갈 수 있습니다.",
    ...
    private void OnClickMailBoxButton()
    {
        if (isFlashing)
        {
            flashTokenSource?.Cancel();
        ...
        mailBoxButton.onClick.RemoveListener(OnClickMailBoxButton);
    }
}
